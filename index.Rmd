...
* Created a project [workspace](https://rstudio.cloud/project/255745) that could be used to build capcaity in R programming so that the eTool could be adapted in country.

## Agenda Items

1. New form created - go over
2. Discuss conversation with Theresa and RStudio Cloud test workspace.
3. Paymnet delay. Need status update, can provide further info if necessary. 
...
* Installed RStudio Connect QuickStart. This is essentially a trial version of RStudio Connect that you run locally on your machine rather than on a server. This is the service I would recommend to CDC as other analytics tools are essentially for reporting only. You can develop reports and administrate access to those reports, however users/viewers cannot alter underlying data using those reporting tools. 
