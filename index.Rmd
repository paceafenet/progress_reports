---
# BE SURE TO OUTPUT TWICE - Once w/o TOC and then save to pdf via Chrome, then with TOC as an html
...
2.	Support the deployment of an ASLM administered reporting platform
  ...
  * Began configuring the server to run the eTool code.
ADD MORE HERE ABOUT REVISION OF REPORTS AND DEPLOYMENT NOTES

## Challenges

* Need feedback on the forms and must have someone attempt to use them
* While it is not a blocker for doing work at present. I would really like to replace my credit card on Amazon Web Services with another payment method.
* Currently having issues configuring the environment. The free to use tier infrastructure may have issues installing packages I will need in the environment. Will investigate further. In order to bypass this issue as well as the payment issue, we may want to provision a paid shinyapps.io account. The environment would already be set to accept the eTool code, authentication comes with the Standard and Professional accounts, and 10,000 active hours comes with the professional tier which is enough to run an application constant with hours to spare for other testing applications. Additionally, this service could be paid up front which allows me to invoice the cost of the service as opposed to running a pay as you go platform like Amazon Web Services. 
